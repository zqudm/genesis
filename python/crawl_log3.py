...
def process_repo(repo_gh_path):
    ...
    #ret = 0;
    #system("mkdir "+ orig_src_dir)
    #system("cp -r " + repo_gh_path+"/ " + orig_src_dir); 
    #src_d = orig_src_dir +"/" + repo_gh_path;
    #if (github_user == ""):
    ...
    #p = subprocess.Popen("git log", stdout = subprocess.PIPE, cwd=repo_gh_path,shell=True);
    p = subprocess.Popen("git log -i --grep=\"fix\\|bug\\|exception\\|error\\|issue\"  --all-match --oneline", stdout = subprocess.PIPE, cwd=repo_gh_path, shell=True);
    (out, err) = p.communicate();
    ret = p.returncode;
    #print src_d
    
    out_f = out_dir + "/" + "xy4" + "_revs"
    out_src_dir = out_dir + "/" + "xy" + "_po"
    system("rm -rf " + out_src_dir)
    system("mkdir " + out_src_dir)
    f = open(out_f, "w") 
    global orig_src_dir
    orig_src_dir = repo_gh_path;
    for line in out.split("\n"):
        if line != "":
            process_commit(line.split(" ")[0], resDirName, out_f, out_src_dir, f, repo_gh_path);
            #print commit_id
            #print line
            #break;
    f.flush()
...
def process_commit(commit_id, resDirName, out_f, out_src_dir, f,repo_gh_path):
    ...
    tmp1= dir+"/"+src1_dir+ "/"+src_file
    tmp2= dir+"/"+ src2_dir+"/"+src_file
    print tmp1
    print tmp2
    myproject_dir = "/root/myproject"
    diff_cmd = 'mvn exec:java -q -Dexec.mainClass="ravens.diff.Differ"'+' -Dexec.args="' + tmp1 +" "+ tmp2+'"'
    
    p = subprocess.Popen([diff_cmd],stdout=subprocess.PIPE,cwd=myproject_dir,shell=True)
    (out, err) = p.communicate()
