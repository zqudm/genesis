...
def process_repo(repo_gh_path):
    ...
    #ret = 0;
    repo_gh_path = repo_gh_path.strip();
    repo_gh_path = repo_gh_path+".git"
    print repo_gh_path;

    gitcmd = "git clone https://github.com" + repo_gh_path;
    print gitcmd;
    #if (github_user == ""):
    ...
    #    p = subprocess.Popen("git log -i --grep=\"fix\\|error\\|exception\" --grep=\"bug\\|issue\" --all-match --oneline", stdout = subprocess.PIPE, cwd=orig_src_dir, shell=True);
        #p = subprocess.Popen("git log", stdout = subprocess.PIPE, cwd=src_d, shell=True);
