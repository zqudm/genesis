...
from repo_handler_xy import create_repo_handler
import MySQLdb
...
def unmarshal_reponame(reponame):
    ...
    for c in reponame[1:]:
        if (c == "_"):
            ...
            ret += c;
    return ret;
def repo2master(reponame):
   # print reponame;
   return reponame[reponame.rfind("_")+1:];
...
def rewrite_pair(repo_dir1, repo_dir2, src_file, file1, file2):
    ...
    ret = system(cmd);
    if ret != 0:
       # print repo_dir1, repos_dir2, src_file, file1, file2;
        exit(0)
    if (ret != 0):
        return 2;
...
def process_repo(repoaddr, rname, out_dir, revrec):
    print rname, repoaddr;
    rname=repo2master(rname)+"-master";
    print rname;
    #_git= "git clone --bare https://github.com" + repoaddr;
    #system(_git);

    ##system("ls "+ rname+"/.git")
    #system("mkdir" + "  "+rname+"/.git");
    #system("mv  "+  repoaddr+".git/*  " +  rname+"/.git")
    #system(_git); 
    #exit(0)
    #print "wget --spider https://github.com" + repoaddr + "/archive/master.zip"
    ...
    #else:
    #   system("rm -rf "+ dir0+"/*");
    #system("wget https://github.com" + repoaddr + "/archive/master.zip"+ " " + dir0);
    #print repoaddr;
    #system("rm -rf " + rname);
    #if github_user == "":
